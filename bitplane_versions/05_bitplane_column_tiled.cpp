// Bitplane version 05: row-sum ring buffer with column tiling.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 03, the 5-slot row-sum ring is tile-width rather
// than full-row width. For 32768-wide grids this keeps the ring comfortably
// below L1d size while preserving the same per-generation stencil work.
...
#ifndef DEFAULT_TILE_WORDS
#define DEFAULT_TILE_WORDS 128
#endif
static constexpr int TILE_WORDS = DEFAULT_TILE_WORDS; // 128 words = 8192 cells.
...
static void step_rows_bitplane(const BitGrid& src, BitGrid& dst, int y0, int y1)
{
    ...
    const int ymask = N - 1;
    const int tile_words = std::min(rw, TILE_WORDS);

    std::vector<uint64_t> adult_tmp(tile_words);
    std::vector<uint64_t> rowsum_store(5 * 3 * (size_t)tile_words);
    uint64_t* rs0[5];
    uint64_t* rs1[5];
    uint64_t* rs2[5];
    for (int i = 0; i < 5; ++i) {
        rs0[i] = rowsum_store.data() + (size_t)(3 * i + 0) * tile_words;
        rs1[i] = rowsum_store.data() + (size_t)(3 * i + 1) * tile_words;
        rs2[i] = rowsum_store.data() + (size_t)(3 * i + 2) * tile_words;
    }

    for (int ws = 0; ws < rw; ws += tile_words) {
        const int we = std::min(ws + tile_words, rw);
        const int tw = we - ws;
        ...
        auto fill_slot = [&](int src_y, int slot) {
            ...
            for (int w = 0; w < tw; ++w) {
                adult_tmp[w] = s0[ws + w] & s1[ws + w];
            }

            const uint64_t prev_edge = s0[ws == 0 ? rw - 1 : ws - 1] &
                                       s1[ws == 0 ? rw - 1 : ws - 1];
            const uint64_t next_edge = s0[we == rw ? 0 : we] &
                                       s1[we == rw ? 0 : we];

            if (tw == 1) {
                row_sum_5_word_neighbors(prev_edge, adult_tmp[0], next_edge,
                                         rs0[slot][0], rs1[slot][0], rs2[slot][0]);
                return;
            }

            row_sum_5_word_neighbors(prev_edge, adult_tmp[0], adult_tmp[1],
                                     rs0[slot][0], rs1[slot][0], rs2[slot][0]);

            for (int w = 1; w < tw - 1; ++w) {
                row_sum_5_word_neighbors(adult_tmp[w - 1], adult_tmp[w], adult_tmp[w + 1],
                                         rs0[slot][w], rs1[slot][w], rs2[slot][w]);
            }

            row_sum_5_word_neighbors(adult_tmp[tw - 2], adult_tmp[tw - 1], next_edge,
                                     rs0[slot][tw - 1], rs1[slot][tw - 1], rs2[slot][tw - 1]);
        };
        ...
        for (int y = y0; y < y1; ++y) {
            ...
            for (int w = 0; w < tw; ++w) {
                ...
                const int gw = ws + w;
                const uint64_t s0 = center0[gw];
                const uint64_t s1 = center1[gw];
                ...
                out1[gw] = egg | juvenile | survive;
                out0[gw] = birth | juvenile | survive;
            }
