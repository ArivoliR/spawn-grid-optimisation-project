// Bitplane version 04: bitplane ring with tree-reduced count chains.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 03, horizontal 5-input row sums and vertical
// 5-row combines use tree reductions instead of serial ripple-add chains.
...
static inline void popcount5_masks(uint64_t m0, uint64_t m1, uint64_t m2,
                                   uint64_t m3, uint64_t m4,
                                   uint64_t& out0, uint64_t& out1, uint64_t& out2)
{
    const uint64_t s_a = m0 ^ m1;
    const uint64_t c_a = m0 & m1;
    const uint64_t s_b = m2 ^ m3;
    const uint64_t c_b = m2 & m3;

    const uint64_t t = s_a ^ s_b;
    out0 = t ^ m4;
    const uint64_t c_t = (s_a & s_b) | (t & m4);

    const uint64_t s_c = c_a ^ c_b;
    const uint64_t c_c = c_a & c_b;
    out1 = s_c ^ c_t;
    out2 = c_c | (s_c & c_t);
}
...
{
    popcount5_masks(shifted_adult_word(adult, rw, w, -2),
                    shifted_adult_word(adult, rw, w, -1),
                    shifted_adult_word(adult, rw, w,  0),
                    shifted_adult_word(adult, rw, w,  1),
                    shifted_adult_word(adult, rw, w,  2),
                    r0, r1, r2);
}
...
static inline void row_sum_5_word_neighbors(uint64_t prev, uint64_t curr, uint64_t next,
                                            uint64_t& r0, uint64_t& r1, uint64_t& r2)
{
    const uint64_t m0 = (curr << 2) | (prev >> 62);
    const uint64_t m1 = (curr << 1) | (prev >> 63);
    const uint64_t m2 = curr;
    const uint64_t m3 = (curr >> 1) | (next << 63);
    const uint64_t m4 = (curr >> 2) | (next << 62);
    popcount5_masks(m0, m1, m2, m3, m4, r0, r1, r2);
}

static inline void add_shift1(uint64_t a0, uint64_t a1, uint64_t a2,
                              uint64_t& c0, uint64_t& c1, uint64_t& c2,
                              uint64_t& c3, uint64_t& c4)
{
    (void)c0;
    uint64_t carry = c1 & a0;
    c1 ^= a0;

    const uint64_t c2xa1 = c2 ^ a1;
    const uint64_t n2 = c2xa1 ^ carry;
    carry = (c2 & a1) | (carry & c2xa1);
    c2 = n2;

    const uint64_t c3xa2 = c3 ^ a2;
    const uint64_t n3 = c3xa2 ^ carry;
    carry = (c3 & a2) | (carry & c3xa2);
    c3 = n3;

    c4 ^= carry;
}

static inline void add_shift2(uint64_t a0, uint64_t a1, uint64_t a2,
                              uint64_t& c0, uint64_t& c1, uint64_t& c2,
                              uint64_t& c3, uint64_t& c4)
{
    (void)c0;
    (void)c1;
    uint64_t carry = c2 & a0;
    c2 ^= a0;

    const uint64_t c3xa1 = c3 ^ a1;
    const uint64_t n3 = c3xa1 ^ carry;
    carry = (c3 & a1) | (carry & c3xa1);
    c3 = n3;

    c4 ^= a2 ^ carry;
}

static inline void combine_rowsums_5(uint64_t a0, uint64_t a1, uint64_t a2,
                                     uint64_t b0, uint64_t b1, uint64_t b2,
                                     uint64_t c0_in, uint64_t c1_in, uint64_t c2_in,
                                     uint64_t d0, uint64_t d1, uint64_t d2,
                                     uint64_t e0, uint64_t e1, uint64_t e2,
                                     uint64_t& c0, uint64_t& c1, uint64_t& c2,
                                     uint64_t& c3, uint64_t& c4)
{
    uint64_t p0_0, p0_1, p0_2;
    uint64_t p1_0, p1_1, p1_2;
    uint64_t p2_0, p2_1, p2_2;

    popcount5_masks(a0, b0, c0_in, d0, e0, p0_0, p0_1, p0_2);
    popcount5_masks(a1, b1, c1_in, d1, e1, p1_0, p1_1, p1_2);
    popcount5_masks(a2, b2, c2_in, d2, e2, p2_0, p2_1, p2_2);
    ...
    add_shift1(p1_0, p1_1, p1_2, c0, c1, c2, c3, c4);
    add_shift2(p2_0, p2_1, p2_2, c0, c1, c2, c3, c4);
}
...
static void step_rows_bitplane(const BitGrid& src, BitGrid& dst, int y0, int y1)
{
    ...
    for (int y = y0; y < y1; ++y) {
        ...
        for (int w = 0; w < rw; ++w) {
            ...
            combine_rowsums_5(rs0[0][w], rs1[0][w], rs2[0][w],
                              rs0[1][w], rs1[1][w], rs2[1][w],
                              rs0[2][w], rs1[2][w], rs2[2][w],
                              rs0[3][w], rs1[3][w], rs2[3][w],
                              rs0[4][w], rs1[4][w], rs2[4][w],
                              c0, c1, c2, c3, c4);
            ...
            const uint64_t birth_ok = nc4 & ~c3 & ((c0 & c1 & ~c2) | (~c1 & c2));
            const uint64_t survive_ok = nc4 & ((~c3 & c2) | (c3 & ~c2 & ~c1));

            const uint64_t birth = empty & birth_ok;
            const uint64_t survive = adult & survive_ok;
