// Bitplane version 06: NEON ripple version of the best scalar bitplane kernel.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 03, the hot row-sum and vertical combine loops
// process two uint64_t words at a time with NEON uint64x2_t vectors. Edge words
// remain scalar so toroidal word-boundary handling stays straightforward.
    ...
    for (int y = y0; y < y1; ++y) {
        ...
        for (; w + 1 < rw; w += 2) {
            ...
            for (int slot = 0; slot < 5; ++slot) {
                add_rowsum_to_count_v(vld1q_u64(rs0[slot] + w),
                                      vld1q_u64(rs1[slot] + w),
                                      vld1q_u64(rs2[slot] + w),
                                      c0, c1, c2, c3, c4);
            }
            ...
            subtract_mask_from_count_v(adult, c0, c1, c2, c3, c4);
            ...
            const uint64x2_t survive = vandq_u64(adult, vorrq_u64(vorrq_u64(vorrq_u64(eq4, eq5), vorrq_u64(eq6, eq7)), vorrq_u64(eq8, eq9)));
