                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    for (int y = y0 + 1; y < y1; ++y) {
        ...
        // vb30: peel the boundary pair out so the main loop's
        // `adult_next_1 = load(r + 2)` is unconditional (no `(r2 < R_REGS) ? ...
        // : load(0)` ternary inside the hot path). On Neoverse-V2 the conditional
        // forced a stack reload of the wrap-around register every iteration
        // (visible in `perf annotate`). Boundary tail is handled below.
        //
        // bnd_adult is the adult bits for column register 0 of the entering row,
        // used as `adult_next_1` for the final pair.
        const uint8x16_t bnd_adult = load_new_adult(0);

        // Body: r = 0, 2, ..., R_REGS - 4. Loads adult_next_1 from r + 2 unconditionally.
        int r = 0;
        for (; r + 2 < R_REGS; r += 2) {
            const size_t off0 = (size_t)r * 16;
            const size_t off1 = (size_t)(r + 1) * 16;
        ...
        // Peeled boundary pair (r = R_REGS - 2). adult_next_1 is the wrap-around bnd.
        if (r + 1 < R_REGS) {
            const size_t off0 = (size_t)r * 16;
            const size_t off1 = (size_t)(r + 1) * 16;
            const uint8x16_t adult_next_0 = load_new_adult(r + 1);
            const uint8x16_t adult_next_1 = bnd_adult;
        ...
        // Single-column tail for odd R_REGS (uncommon since grids are power-of-2).
        for (; r < R_REGS; ++r) {
            const size_t off = (size_t)r * 16;
