                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    for (int y = y0 + 1; y < y1; ++y) {
        ...
        // vb32: 4-column unroll for ILP. Four independent V5 carry chains
        // expose more parallel work to the OOO engine. Trades register
        // pressure (40+ live vectors at peak) for higher per-cycle issue.
        int r = 0;
        for (; r + 3 < R_REGS; r += 4) {
            const size_t off0 = (size_t)(r + 0) * 16;
            const size_t off1 = (size_t)(r + 1) * 16;
            const size_t off2 = (size_t)(r + 2) * 16;
            const size_t off3 = (size_t)(r + 3) * 16;

            const int r4 = r + 4;
            const uint8x16_t adult_next_0 = load_new_adult(r + 1);
            const uint8x16_t adult_next_1 = load_new_adult(r + 2);
            const uint8x16_t adult_next_2 = load_new_adult(r + 3);
            const uint8x16_t adult_next_3 =
                (r4 < R_REGS) ? load_new_adult(r4) : load_new_adult(0);
            ...
            const Sum3 h_new_2 = horizontal_window_sum(adult_next_0, adult_next_1,  adult_next_2);
            const Sum3 h_new_3 = horizontal_window_sum(adult_next_1, adult_next_2,  adult_next_3);

            V5 v0 = load_v5_interleaved(v_data, off0);
            V5 v1 = load_v5_interleaved(v_data, off1);
            V5 v2 = load_v5_interleaved(v_data, off2);
            V5 v3 = load_v5_interleaved(v_data, off3);

            const Sum3 h_old_0 = { vld1q_u8(h_out_0 + off0), vld1q_u8(h_out_1 + off0), vld1q_u8(h_out_2 + off0) };
            const Sum3 h_old_1 = { vld1q_u8(h_out_0 + off1), vld1q_u8(h_out_1 + off1), vld1q_u8(h_out_2 + off1) };
            const Sum3 h_old_2 = { vld1q_u8(h_out_0 + off2), vld1q_u8(h_out_1 + off2), vld1q_u8(h_out_2 + off2) };
            const Sum3 h_old_3 = { vld1q_u8(h_out_0 + off3), vld1q_u8(h_out_1 + off3), vld1q_u8(h_out_2 + off3) };

            v0 = sub_v5_h3(v0, h_old_0);
            v1 = sub_v5_h3(v1, h_old_1);
            v2 = sub_v5_h3(v2, h_old_2);
            v3 = sub_v5_h3(v3, h_old_3);
            v0 = add_v5_h3(v0, h_new_0);
            v1 = add_v5_h3(v1, h_new_1);
            v2 = add_v5_h3(v2, h_new_2);
            v3 = add_v5_h3(v3, h_new_3);

            store_v5_interleaved(v_data, off0, v0);
            store_v5_interleaved(v_data, off1, v1);
            store_v5_interleaved(v_data, off2, v2);
            store_v5_interleaved(v_data, off3, v3);

            vst1q_u8(h_write_0 + off0, h_new_0.h0); vst1q_u8(h_write_1 + off0, h_new_0.h1); vst1q_u8(h_write_2 + off0, h_new_0.h2);
            vst1q_u8(h_write_0 + off1, h_new_1.h0); vst1q_u8(h_write_1 + off1, h_new_1.h1); vst1q_u8(h_write_2 + off1, h_new_1.h2);
            vst1q_u8(h_write_0 + off2, h_new_2.h0); vst1q_u8(h_write_1 + off2, h_new_2.h1); vst1q_u8(h_write_2 + off2, h_new_2.h2);
            vst1q_u8(h_write_0 + off3, h_new_3.h0); vst1q_u8(h_write_1 + off3, h_new_3.h1); vst1q_u8(h_write_2 + off3, h_new_3.h2);

            const uint8x16_t lo0 = vld1q_u8(low_row + off0); const uint8x16_t hi0 = vld1q_u8(high_row + off0);
            const uint8x16_t lo1 = vld1q_u8(low_row + off1); const uint8x16_t hi1 = vld1q_u8(high_row + off1);
            const uint8x16_t lo2 = vld1q_u8(low_row + off2); const uint8x16_t hi2 = vld1q_u8(high_row + off2);
            const uint8x16_t lo3 = vld1q_u8(low_row + off3); const uint8x16_t hi3 = vld1q_u8(high_row + off3);
            const LH out0 = apply_rule_byte(v0, lo0, hi0);
            const LH out1 = apply_rule_byte(v1, lo1, hi1);
            const LH out2 = apply_rule_byte(v2, lo2, hi2);
            const LH out3 = apply_rule_byte(v3, lo3, hi3);
            vst1q_u8(nl_row + off0, out0.low); vst1q_u8(nh_row + off0, out0.high);
            vst1q_u8(nl_row + off1, out1.low); vst1q_u8(nh_row + off1, out1.high);
            vst1q_u8(nl_row + off2, out2.low); vst1q_u8(nh_row + off2, out2.high);
            vst1q_u8(nl_row + off3, out3.low); vst1q_u8(nh_row + off3, out3.high);

            adult_prev = adult_next_2;
            adult_curr = adult_next_3;
        }
        // 2-column unroll tail. Handles the case where R_REGS % 4 in {2, 3}.
        for (; r + 1 < R_REGS; r += 2) {
            const size_t off0 = (size_t)r * 16;
            const size_t off1 = (size_t)(r + 1) * 16;
            const int r2 = r + 2;
            const uint8x16_t adult_next_0 = load_new_adult(r + 1);
            const uint8x16_t adult_next_1 = (r2 < R_REGS) ? load_new_adult(r2) : load_new_adult(0);
            const Sum3 h_new_0 = horizontal_window_sum(adult_prev, adult_curr, adult_next_0);
            const Sum3 h_new_1 = horizontal_window_sum(adult_curr, adult_next_0, adult_next_1);
            V5 v0 = load_v5_interleaved(v_data, off0);
            V5 v1 = load_v5_interleaved(v_data, off1);
            const Sum3 h_old_0 = { vld1q_u8(h_out_0 + off0), vld1q_u8(h_out_1 + off0), vld1q_u8(h_out_2 + off0) };
            const Sum3 h_old_1 = { vld1q_u8(h_out_0 + off1), vld1q_u8(h_out_1 + off1), vld1q_u8(h_out_2 + off1) };
            v0 = sub_v5_h3(v0, h_old_0); v1 = sub_v5_h3(v1, h_old_1);
            v0 = add_v5_h3(v0, h_new_0); v1 = add_v5_h3(v1, h_new_1);
            store_v5_interleaved(v_data, off0, v0); store_v5_interleaved(v_data, off1, v1);
            vst1q_u8(h_write_0 + off0, h_new_0.h0); vst1q_u8(h_write_1 + off0, h_new_0.h1); vst1q_u8(h_write_2 + off0, h_new_0.h2);
            vst1q_u8(h_write_0 + off1, h_new_1.h0); vst1q_u8(h_write_1 + off1, h_new_1.h1); vst1q_u8(h_write_2 + off1, h_new_1.h2);
            const uint8x16_t lo0 = vld1q_u8(low_row + off0); const uint8x16_t hi0v = vld1q_u8(high_row + off0);
            const uint8x16_t lo1 = vld1q_u8(low_row + off1); const uint8x16_t hi1v = vld1q_u8(high_row + off1);
            const LH outL = apply_rule_byte(v0, lo0, hi0v);
            const LH outR = apply_rule_byte(v1, lo1, hi1v);
            vst1q_u8(nl_row + off0, outL.low); vst1q_u8(nh_row + off0, outL.high);
            vst1q_u8(nl_row + off1, outR.low); vst1q_u8(nh_row + off1, outR.high);
            adult_prev = adult_next_0; adult_curr = adult_next_1;
        }
        // Single-column scalar tail.
        for (; r < R_REGS; ++r) {
            const size_t off = (size_t)r * 16;
