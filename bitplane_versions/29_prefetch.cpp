                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    for (int y = y0 + 1; y < y1; ++y) {
        ...
        int r = 0;
        // vb29: prefetch the entering row, the v scratch, and the ring slot
        // ~4 column-registers ahead so the OOO engine can pipeline loads
        // around the boolean / carry-chain work.
        ...
        for (; r + 1 < R_REGS; r += 2) {
            ...
            if (r + PF_AHEAD < R_REGS) {
                const size_t pf_off = (size_t)(r + PF_AHEAD) * 16;
                __builtin_prefetch(new_lo + pf_off, 0, 1);
                __builtin_prefetch(new_hi + pf_off, 0, 1);
                __builtin_prefetch(v_data + pf_off * 5, 0, 1);
                __builtin_prefetch(h_out_0 + pf_off, 0, 1);
                __builtin_prefetch(low_row + pf_off, 0, 1);
                __builtin_prefetch(high_row + pf_off, 0, 1);
            }
