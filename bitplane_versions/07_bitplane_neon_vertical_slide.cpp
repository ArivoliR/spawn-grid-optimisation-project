// Bitplane version 07: NEON ripple with vertically sliding 5-row count.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 06, this keeps a running vertical 5-row sum:
// V[y+1] = V[y] - H[y-2] + H[y+3]. The count includes the center cell, so
// transition thresholds are adjusted and the center-adult subtract is removed.
...
static void step_rows_bitplane(const BitGrid& src, BitGrid& dst, int y0, int y1)
{
    ...
    std::vector<uint64_t> vcount_store(5 * (size_t)rw);
    uint64_t* rs0[5];
    uint64_t* rs1[5];
    uint64_t* rs2[5];
    uint64_t* vc0 = vcount_store.data() + 0 * (size_t)rw;
    uint64_t* vc1 = vcount_store.data() + 1 * (size_t)rw;
    uint64_t* vc2 = vcount_store.data() + 2 * (size_t)rw;
    uint64_t* vc3 = vcount_store.data() + 3 * (size_t)rw;
    uint64_t* vc4 = vcount_store.data() + 4 * (size_t)rw;
    for (int i = 0; i < 5; ++i) {
    ...
    auto fill_slot = [&](int src_y, int slot) {
        ...
        for (; aw + 1 < rw; aw += 2) {
            ...
            vst1q_u64(adult_tmp.data() + aw, vandq_u64(a0, a1));
        }
        for (; aw < rw; ++aw) {
        ...
        int w = 1;
        for (; w + 1 < rw - 1; w += 2) {
            const uint64x2_t prev = vld1q_u64(adult_tmp.data() + w - 1);
            const uint64x2_t curr = vld1q_u64(adult_tmp.data() + w);
            const uint64x2_t next = vld1q_u64(adult_tmp.data() + w + 1);
            uint64x2_t r0, r1, r2;
            row_sum_5_pair_neighbors(prev, curr, next, r0, r1, r2);
            vst1q_u64(rs0[slot] + w, r0);
            vst1q_u64(rs1[slot] + w, r1);
            vst1q_u64(rs2[slot] + w, r2);
        }
    ...
    for (int y = y0; y < y1; ++y) {
        ...
        for (; w + 1 < rw; w += 2) {
            ...
            const uint64x2_t nc0 = vnotq_u64(c0);
            const uint64x2_t eq3 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, nc3), nc2), c1), c0);
            const uint64x2_t eq4 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, nc3), c2), nc1), nc0);
            const uint64x2_t eq5 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, nc3), c2), nc1), c0);
            const uint64x2_t eq6 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, nc3), c2), c1), nc0);
            const uint64x2_t eq7 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, nc3), c2), c1), c0);
            const uint64x2_t eq8 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, c3), nc2), nc1), nc0);
            const uint64x2_t eq9 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, c3), nc2), nc1), c0);
            const uint64x2_t eq10 = vandq_u64(vandq_u64(vandq_u64(vandq_u64(nc4, c3), nc2), c1), nc0);

            const uint64x2_t birth = vandq_u64(empty, vorrq_u64(vorrq_u64(eq3, eq4), eq5));
            const uint64x2_t survive = vandq_u64(adult, vorrq_u64(vorrq_u64(vorrq_u64(eq5, eq6), vorrq_u64(eq7, eq8)), vorrq_u64(eq9, eq10)));
        ...
        for (; w < rw; ++w) {
            ...
            const uint64_t eq10 = nc4 & c3 & nc2 & c1 & nc0;

            const uint64_t birth = empty & (eq3 | eq4 | eq5);
            const uint64_t survive = adult & (eq5 | eq6 | eq7 | eq8 | eq9 | eq10);
        ...
        for (; uw + 1 < rw; uw += 2) {
            ...
            subtract_rowsum_from_count_v(vld1q_u64(rs0[tail] + uw),
                                         vld1q_u64(rs1[tail] + uw),
                                         vld1q_u64(rs2[tail] + uw),
                                         c0, c1, c2, c3, c4);
            vst1q_u64(vc0 + uw, c0);
            vst1q_u64(vc1 + uw, c1);
            vst1q_u64(vc2 + uw, c2);
        ...
        for (; uw < rw; ++uw) {
            subtract_rowsum_from_count(rs0[tail][uw], rs1[tail][uw], rs2[tail][uw],
        ...
        for (; uw + 1 < rw; uw += 2) {
            ...
            uint64x2_t c3 = vld1q_u64(vc3 + uw);
            uint64x2_t c4 = vld1q_u64(vc4 + uw);
            add_rowsum_to_count_v(vld1q_u64(rs0[tail] + uw),
                                  vld1q_u64(rs1[tail] + uw),
                                  vld1q_u64(rs2[tail] + uw),
                                  c0, c1, c2, c3, c4);
            vst1q_u64(vc0 + uw, c0);
        ...
        for (; uw < rw; ++uw) {
            add_rowsum_to_count(rs0[tail][uw], rs1[tail][uw], rs2[tail][uw],
                                vc0[uw], vc1[uw], vc2[uw], vc3[uw], vc4[uw]);
        }
