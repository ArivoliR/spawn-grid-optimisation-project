// Bitplane version 11: NEON temporal row-stripe blocking experiment.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 10, this advances a full-width row stripe for up
// to four generations while the stripe and its vertical halo rows are resident
// in per-worker scratch buffers. This still evaluates every cell for every
// generation; it only changes the memory access pattern.
...
static constexpr int TEMPORAL_K = 4;
static constexpr int STRIPE_ROWS = 64;
...
struct BitGrid {
    ...
    void resize_rows(int row_count_power_of_two, int words)
    {
        n = row_count_power_of_two;
        row_words = words;
        s0.assign((size_t)n * row_words, 0);
        s1.assign((size_t)n * row_words, 0);
        advise_huge_pages(s0.data(), s0.size() * sizeof(uint64_t));
                                                ...
                                                uint64x2_t& c4)
{
    ...
    const uint64x2_t c1xr1 = veorq_u64(c1, r1);
    const uint64x2_t d1 = veorq_u64(c1xr1, borrow);
    borrow = vorrq_u64(vandq_u64(vnotq_u64(c1), r1),
                       vandq_u64(vnotq_u64(c1xr1), borrow));
    c1 = d1;

    const uint64x2_t c2xr2 = veorq_u64(c2, r2);
    const uint64x2_t d2 = veorq_u64(c2xr2, borrow);
...
struct ThreadScratch {
    std::vector<uint64_t> adult_tmp;
    std::vector<uint64_t> h_store;
    std::vector<uint64_t> vcount_store;
    BitGrid slab_a;
    BitGrid slab_b;
    ...
    void resize(int rw)
    {
        ...
        slab_a.resize_rows(128, rw);
        slab_b.resize_rows(128, rw);
...
static void load_stripe_slab(const BitGrid& src, BitGrid& slab,
                             int stripe_y, int inner_rows, int steps)
{
    const int rw = src.row_words;
    const int ymask = src.n - 1;
    const int halo = 2 * steps;
    const int slab_rows = inner_rows + 2 * halo;

    for (int sy = 0; sy < slab_rows; ++sy) {
        const int gy = (stripe_y - halo + sy) & ymask;
        std::memcpy(slab.row0(sy), src.row0(gy), (size_t)rw * sizeof(uint64_t));
        std::memcpy(slab.row1(sy), src.row1(gy), (size_t)rw * sizeof(uint64_t));
    }
}

static void store_stripe_slab(const BitGrid& slab, BitGrid& dst,
                              int stripe_y, int inner_rows, int steps)
{
    const int rw = dst.row_words;
    const int halo = 2 * steps;

    for (int sy = 0; sy < inner_rows; ++sy) {
        std::memcpy(dst.row0(stripe_y + sy), slab.row0(halo + sy), (size_t)rw * sizeof(uint64_t));
        std::memcpy(dst.row1(stripe_y + sy), slab.row1(halo + sy), (size_t)rw * sizeof(uint64_t));
    }
}

static void process_temporal_range(const BitGrid& src, BitGrid& dst,
                                   int y0, int y1, int steps,
                                   ThreadScratch& scratch)
{
    for (int stripe_y = y0; stripe_y < y1; stripe_y += STRIPE_ROWS) {
        const int inner_rows = std::min(STRIPE_ROWS, y1 - stripe_y);
        const int slab_rows = inner_rows + 4 * steps;

        load_stripe_slab(src, scratch.slab_a, stripe_y, inner_rows, steps);

        BitGrid* slab_cur = &scratch.slab_a;
        BitGrid* slab_next = &scratch.slab_b;
        for (int g = 0; g < steps; ++g) {
            const int out_y0 = 2 * (g + 1);
            const int out_y1 = slab_rows - 2 * (g + 1);
            step_rows_bitplane(*slab_cur, *slab_next, out_y0, out_y1, scratch);
            std::swap(slab_cur, slab_next);
        }

        store_stripe_slab(*slab_cur, dst, stripe_y, inner_rows, steps);
    }
}
...
int main(int argc, char* argv[])
{
    ...
    for (unsigned t = 1; t < T; ++t) {
        pool.emplace_back([&, t]() {
            for (;;) {
                ...
                if (stop) return;
                process_temporal_range(*shared_src, *shared_dst,
                                       row_lo[t], row_hi[t], shared_steps, scratch[t]);
    ...
    for (int gen = 0; gen < generations; gen += shared_steps) {
        shared_steps = std::min(TEMPORAL_K, generations - gen);
        shared_src = cur;
        shared_dst = next;
        bar_start.arrive_and_wait();
        process_temporal_range(*cur, *next, row_lo[0], row_hi[0], shared_steps, scratch[0]);
        bar_done.arrive_and_wait();
        std::swap(cur, next);
    }
