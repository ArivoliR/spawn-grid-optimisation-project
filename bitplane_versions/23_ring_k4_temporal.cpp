...
struct ThreadScratch {
    ...
    void resize(int row_bytes, int slab_rows_inclusive_ghost)
    {
        h_ring.resize(6 * 3 * (size_t)row_bytes);
        ...
        const size_t bytes = (size_t)local_rows * (size_t)row_bytes;
                               ...
                               ThreadScratch& scratch)
{
    ...
    // 1. Ghost copy: (slab + 2*ghost) rows from src into lbuf_a, with toroidal wrap.
    for (int local_y = 0; local_y < local_h; ++local_y) {
        const int global_y = (y_slab_start - ghost + local_y + N) & ymask;
        std::memcpy(la_low  + (size_t)local_y * row_bytes,
                    src.row0(global_y), (size_t)row_bytes);
        std::memcpy(la_high + (size_t)local_y * row_bytes,
                    src.row1(global_y), (size_t)row_bytes);
    }

    // 2. Run K_this generations on local buffers. Window shrinks by 2 rows / side / gen.
    uint8_t* cur_low  = la_low;
    uint8_t* cur_high = la_high;
    uint8_t* nxt_low  = lb_low;
    uint8_t* nxt_high = lb_high;

    for (int k = 1; k <= K_this; ++k) {
        const int y0 = 2 * k;
        const int y1 = local_h - 2 * k;
        step_rows_bitplane_local(cur_low, cur_high, nxt_low, nxt_high,
                                 row_bytes, y0, y1, scratch);
        std::swap(cur_low, nxt_low);
        std::swap(cur_high, nxt_high);
    }
    // After K_this gens, cur_* holds final values; central slab is at rows [ghost, ghost + slab_h).

    // 3. Write the slab back to global dst.
    for (int local_y = ghost; local_y < ghost + slab_h; ++local_y) {
        const int global_y = (y_slab_start + (local_y - ghost)) & ymask;
        std::memcpy(dst.row0(global_y),
                    cur_low  + (size_t)local_y * row_bytes, (size_t)row_bytes);
        std::memcpy(dst.row1(global_y),
                    cur_high + (size_t)local_y * row_bytes, (size_t)row_bytes);
    }
}
