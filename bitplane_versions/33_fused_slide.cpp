...
// vb33: fused slide = sub + add interleaved at the bit level.
// Computes V' = V - h_old + h_new in a single ripple.
//
// Why this is faster: the standalone sub_v5_h3 has a depth-5 borrow chain,
// and add_v5_h3 has a depth-5 carry chain. Called sequentially, the add
// depends on the entire sub output → total dep depth 10. Interleaved by
// bit, both chains advance one stage per bit position; the carry chain
// only feeds forward to itself, and the borrow chain only feeds forward
// to itself (and into tmp_b, which the carry chain also consumes). Total
// dep depth becomes ~6 instead of 10.
//
// The h_old/h_new are 3-bit values (h.h0/h.h1/h.h2), so the implicit bits
// at positions 3 and 4 of both are zero — that simplifies the upper bits
// of the ripple (no eor3/bsl needed there).
static inline V5 slide_v5_h3(V5 v, Sum3 h_old, Sum3 h_new)
{
    // Bit 0
    const uint8x16_t tmp0  = veorq_u8(v.b0, h_old.h0);
    const uint8x16_t bor0  = vbicq_u8(h_old.h0, v.b0);             // ~v.b0 & h_old.h0
    const uint8x16_t out0  = veorq_u8(tmp0, h_new.h0);             // diff0 ^ h_new.h0
    const uint8x16_t car0  = vandq_u8(tmp0, h_new.h0);

    // Bit 1 — full sub + full add stage, both chains advance.
    const uint8x16_t bxh1  = veorq_u8(v.b1, h_old.h1);
    const uint8x16_t tmp1  = veor3q_u8(v.b1, h_old.h1, bor0);      // diff bit 1
    const uint8x16_t bor1  = vbslq_u8(bxh1, h_old.h1, bor0);       // borrow MAJ
    const uint8x16_t tx1   = veorq_u8(tmp1, h_new.h1);
    const uint8x16_t out1  = veor3q_u8(tmp1, h_new.h1, car0);
    const uint8x16_t car1  = vbslq_u8(tx1, car0, tmp1);            // carry MAJ

    // Bit 2 — same shape as bit 1.
    ...
    // Bit 3 — h_old.h3 = h_new.h3 = 0 (3-bit operands). Borrow MAJ degenerates to
    // (~v.b3 & bor2); carry MAJ degenerates to (tmp3 & car2).
    const uint8x16_t tmp3  = veorq_u8(v.b3, bor2);
    const uint8x16_t bor3  = vbicq_u8(bor2, v.b3);                 // ~v.b3 & bor2
    const uint8x16_t out3  = veorq_u8(tmp3, car2);
    const uint8x16_t car3  = vandq_u8(tmp3, car2);

    // Bit 4 — same: h_old.h4 = h_new.h4 = 0, so just XOR all three.
                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    for (int y = y0 + 1; y < y1; ++y) {
        ...
        for (; r + 1 < R_REGS; r += 2) {
            ...
            // Slide: V -= h_old, V += h_new. Single bit-interleaved ripple
            // (depth-6 vs the depth-10 of sub-then-add).
