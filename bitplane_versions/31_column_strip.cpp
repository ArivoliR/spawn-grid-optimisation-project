...
// =====================================================================
// vb31: column-strip step kernel.
//
// vb27/vb28 iterate row-major: for each row, iterate all column registers.
// V state per column reg is loaded from `v_data` scratch and stored back
// every row — that's 10 vector load/store ops per column reg per row.
// At 32K wide × 8 threads, this is ~84M V-scratch ops per thread per gen
// (~12% of total instructions).
//
// vb31 inverts the iteration: per thread, the outer loop walks column
// strips (S = 2 register columns each); the inner loop walks all band
// rows within that strip. Per-strip:
//   - Initialise V5 for the strip's 2 cols once (sum of 5 H rows).
//   - V5 lives in vector REGISTERS across the entire row loop — no
//     `v_data` load/store per row.
//   - H ring (5 slots × 3 planes × 2 cols = 480 B) is stack-local;
//     small enough that L1 covers it trivially.
//
// Trade-off: per strip we re-do an init pass (5 H rows + V init). At
// R_REGS / S = 1024 strips per thread per gen, init adds ~10 K H computes
// per thread per gen — negligible against 8.4 M steady-state computes
// (<0.2 % overhead).
//
// All the helpers from vb27 (BSL MAJ-folded sum_of_5 / add_v5_h3 /
// sub_v5_h3 / apply_rule_byte) are reused unchanged.
// =====================================================================
                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    // The per-thread h_ring/vcount_store from ThreadScratch are unused here
    // (V is in registers, H ring is per-strip stack-local). Kept allocated
    // for ABI compatibility with the other versions.
    (void)scratch;

    constexpr int S = 2;  // strip width in 16-byte column registers (32 cells wide).

    // Stack-local per-strip H ring. Layout: [slot][plane][col-reg] × 16 bytes.
    // 5 slots × 3 planes × S=2 cols × 16 = 480 bytes. alignas(64) for L1 hit.
    alignas(64) uint8_t local_ring[RING_SLOTS * 3 * S * 16];
    auto h0p = [&](int slot, int col) {
        return local_ring + (size_t)((slot * 3 + 0) * S + col) * 16;
    };
    auto h1p = [&](int slot, int col) {
        return local_ring + (size_t)((slot * 3 + 1) * S + col) * 16;
    };
    auto h2p = [&](int slot, int col) {
        return local_ring + (size_t)((slot * 3 + 2) * S + col) * 16;
    };

    auto load_adult_at = [&](const uint8_t* lo, const uint8_t* hi, int col) -> uint8x16_t {
        const uint8x16_t l = vld1q_u8(lo + (size_t)col * 16);
        const uint8x16_t h = vld1q_u8(hi + (size_t)col * 16);
        return vandq_u8(l, h);
    };

    // -------- Outer loop: walk column strips. --------
    // R_REGS is always even (grid widths are powers of two ≥ 16), so the
    // strip walk covers every column register exactly once.
    for (int c_strip = 0; c_strip < R_REGS; c_strip += S) {
        const int c0 = c_strip;
        const int c1 = c_strip + 1;
        const int c_prev = (c_strip == 0) ? (R_REGS - 1) : (c_strip - 1);
        const int c_next = (c_strip + S >= R_REGS) ? 0 : (c_strip + S);

        // -------- Per-strip init: fill ring slots 0..4 with H_{y0-2..y0+2}. --------
        for (int s = 0; s < RING_SLOTS; ++s) {
            const int src_y = (y0 - 2 + s + N) & ymask;
            const uint8_t* lo = src.row0(src_y);
            const uint8_t* hi = src.row1(src_y);
            const uint8x16_t a_p = load_adult_at(lo, hi, c_prev);
            const uint8x16_t a_0 = load_adult_at(lo, hi, c0);
            const uint8x16_t a_1 = load_adult_at(lo, hi, c1);
            const uint8x16_t a_n = load_adult_at(lo, hi, c_next);
            const Sum3 h_c0 = horizontal_window_sum(a_p, a_0, a_1);
            const Sum3 h_c1 = horizontal_window_sum(a_0, a_1, a_n);
            vst1q_u8(h0p(s, 0), h_c0.h0);
            vst1q_u8(h1p(s, 0), h_c0.h1);
            vst1q_u8(h2p(s, 0), h_c0.h2);
            vst1q_u8(h0p(s, 1), h_c1.h0);
            vst1q_u8(h1p(s, 1), h_c1.h1);
            vst1q_u8(h2p(s, 1), h_c1.h2);
        }

        // -------- Init V state in registers (sum of ring slots 0..4). --------
        V5 v0 = {
            vld1q_u8(h0p(0, 0)), vld1q_u8(h1p(0, 0)), vld1q_u8(h2p(0, 0)),
            vdupq_n_u8(0), vdupq_n_u8(0),
        };
        V5 v1 = {
            vld1q_u8(h0p(0, 1)), vld1q_u8(h1p(0, 1)), vld1q_u8(h2p(0, 1)),
            vdupq_n_u8(0), vdupq_n_u8(0),
        };
        for (int s = 1; s < RING_SLOTS; ++s) {
            const Sum3 h_c0 = { vld1q_u8(h0p(s, 0)), vld1q_u8(h1p(s, 0)), vld1q_u8(h2p(s, 0)) };
            const Sum3 h_c1 = { vld1q_u8(h0p(s, 1)), vld1q_u8(h1p(s, 1)), vld1q_u8(h2p(s, 1)) };
            v0 = add_v5_h3(v0, h_c0);
            v1 = add_v5_h3(v1, h_c1);
        }

        // -------- Apply rule for first output row y0 in this strip. --------
        {
            const uint8_t* lo_y = src.row0(y0);
            const uint8_t* hi_y = src.row1(y0);
            const uint8x16_t s0_lo = vld1q_u8(lo_y + (size_t)c0 * 16);
            const uint8x16_t s0_hi = vld1q_u8(hi_y + (size_t)c0 * 16);
            const uint8x16_t s1_lo = vld1q_u8(lo_y + (size_t)c1 * 16);
            const uint8x16_t s1_hi = vld1q_u8(hi_y + (size_t)c1 * 16);
            const LH out0 = apply_rule_byte(v0, s0_lo, s0_hi);
            const LH out1 = apply_rule_byte(v1, s1_lo, s1_hi);
            uint8_t* nl = dst.row0(y0);
            uint8_t* nh = dst.row1(y0);
            vst1q_u8(nl + (size_t)c0 * 16, out0.low);
            vst1q_u8(nh + (size_t)c0 * 16, out0.high);
            vst1q_u8(nl + (size_t)c1 * 16, out1.low);
            vst1q_u8(nh + (size_t)c1 * 16, out1.high);
        }

        // -------- Slide loop: for each subsequent row in the band. --------
        // V state lives in registers (v0, v1) for the whole loop.
        int tail = 0;
        for (int y = y0 + 1; y < y1; ++y) {
            ...
            // Load adults for the entering row at the 4 needed column positions.
            const uint8x16_t a_p = load_adult_at(new_lo, new_hi, c_prev);
            const uint8x16_t a_0 = load_adult_at(new_lo, new_hi, c0);
            const uint8x16_t a_1 = load_adult_at(new_lo, new_hi, c1);
            const uint8x16_t a_n = load_adult_at(new_lo, new_hi, c_next);

            // Compute new H for the strip's 2 cols.
            const Sum3 h_new_0 = horizontal_window_sum(a_p, a_0, a_1);
            const Sum3 h_new_1 = horizontal_window_sum(a_0, a_1, a_n);

            // Load OLD H from ring[tail] for the 2 cols.
            const Sum3 h_old_0 = {
                vld1q_u8(h0p(tail, 0)),
                vld1q_u8(h1p(tail, 0)),
                vld1q_u8(h2p(tail, 0)),
            ...
            // Slide V (in registers — no v_data round-trip).
            v0 = sub_v5_h3(v0, h_old_0);
            v1 = sub_v5_h3(v1, h_old_1);
            v0 = add_v5_h3(v0, h_new_0);
            v1 = add_v5_h3(v1, h_new_1);

            // Write new H to ring[tail] for future iterations.
            vst1q_u8(h0p(tail, 0), h_new_0.h0);
            vst1q_u8(h1p(tail, 0), h_new_0.h1);
            vst1q_u8(h2p(tail, 0), h_new_0.h2);
            vst1q_u8(h0p(tail, 1), h_new_1.h0);
            vst1q_u8(h1p(tail, 1), h_new_1.h1);
            vst1q_u8(h2p(tail, 1), h_new_1.h2);

            // Apply rule for row y, emit dst.
            const uint8_t* lo_y = src.row0(y);
            const uint8_t* hi_y = src.row1(y);
            const uint8x16_t s0_lo = vld1q_u8(lo_y + (size_t)c0 * 16);
            const uint8x16_t s0_hi = vld1q_u8(hi_y + (size_t)c0 * 16);
            const uint8x16_t s1_lo = vld1q_u8(lo_y + (size_t)c1 * 16);
            const uint8x16_t s1_hi = vld1q_u8(hi_y + (size_t)c1 * 16);
            const LH out0 = apply_rule_byte(v0, s0_lo, s0_hi);
            const LH out1 = apply_rule_byte(v1, s1_lo, s1_hi);
            uint8_t* nl = dst.row0(y);
            uint8_t* nh = dst.row1(y);
            vst1q_u8(nl + (size_t)c0 * 16, out0.low);
            vst1q_u8(nh + (size_t)c0 * 16, out0.high);
            vst1q_u8(nl + (size_t)c1 * 16, out1.low);
            vst1q_u8(nh + (size_t)c1 * 16, out1.high);
