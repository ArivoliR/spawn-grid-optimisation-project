// Bitplane version 15: vb14 + fused vertical slide update.
//
// This starts from vb14 and changes only the hot vertical slide arithmetic.
// Instead of:
//
//   v = sub_v5_h3(v, h_out);
//   v = add_v5_h3(v, h_in);
//
// it computes the same `V - H_out + H_in` update with a fused carry-save plus
// ripple network. This preserves the same O(generations * cells) algorithm and
// row-major memory walk.
//
//   - one uint8x16_t register still represents 128 cells
//   - horizontal x-shifts use vextq_u8 plus byte shifts
//   - the horizontal 5-cell adult count uses FA + FA + HA
//   - the H-row kernel is unrolled by 4 registers
//   - the transition directly derives next low/high bitplanes
//
...
//   - loop-swap to r outer (column-stride access regressed badly)
//   - temporal K=2 slab blocking (correct but slower on target)
//
...
//          bitplane_versions/14_bitplane_byte_h4_compact.cpp -o /tmp/vb14_b64
//
//   g++-14 -std=c++23 -O3 -mcpu=neoverse-v2+sha3 -pthread \
//          -DSPAWN_BLOCK_ROWS=256 \
//          bitplane_versions/14_bitplane_byte_h4_compact.cpp -o /tmp/vb14_b256
...
static inline V5 slide_v5_h3_h3(V5 v, Sum3 h_out, Sum3 h_in)
{
    // Two's-complement -h_out in five bitplanes. h_out is in 0..5, so the
    // high two bits are the sign-extension bits of the negated value.
    const uint8x16_t out01 = vorrq_u8(h_out.h0, h_out.h1);
    const uint8x16_t neg0 = h_out.h0;
    const uint8x16_t neg1 = veorq_u8(h_out.h1, h_out.h0);
    const uint8x16_t neg2 = veorq_u8(h_out.h2, out01);
    const uint8x16_t neg34 = vorrq_u8(out01, h_out.h2);

    // Carry-save add v + h_in + (-h_out).
    const FA csa0 = full_adder_u8(v.b0, h_in.h0, neg0);
    const FA csa1 = full_adder_u8(v.b1, h_in.h1, neg1);
    const FA csa2 = full_adder_u8(v.b2, h_in.h2, neg2);
    const HA csa3 = half_adder_u8(v.b3, neg34);
    const HA csa4 = half_adder_u8(v.b4, neg34);

    // Final add: sum bits plus the carry-save carries shifted left by one.
    const uint8x16_t b0 = csa0.s;
    const HA a1 = half_adder_u8(csa1.s, csa0.c);
    const FA a2 = full_adder_u8(csa2.s, csa1.c, a1.c);
    const FA a3 = full_adder_u8(csa3.s, csa2.c, a2.c);
    const FA a4 = full_adder_u8(csa4.s, csa3.c, a3.c);
    return { b0, a1.s, a2.s, a3.s, a4.s };
}
                               ...
                               int y0, int y1, ThreadScratch& scratch)
{
    ...
    uint8_t* v_b0 = scratch.v_b0;
    uint8_t* v_b1 = scratch.v_b1;
    uint8_t* v_b2 = scratch.v_b2;
    uint8_t* v_b3 = scratch.v_b3;
    uint8_t* v_b4 = scratch.v_b4;
    ...
    for (int b0 = y0; b0 < y1; b0 += BLOCK_ROWS) {
        ...
        // ---- Pass 2: initial V = sum of H[0..4]. Unrolled by 2 in r. ----
        {
            ...
            for (; r + 1 < R_REGS; r += 2) {
                ...
                vst1q_u8(v_b0 + off0, v0.b0);
                vst1q_u8(v_b1 + off0, v0.b1);
                vst1q_u8(v_b2 + off0, v0.b2);
                vst1q_u8(v_b3 + off0, v0.b3);
                vst1q_u8(v_b4 + off0, v0.b4);
                vst1q_u8(v_b0 + off1, v1.b0);
                vst1q_u8(v_b1 + off1, v1.b1);
                vst1q_u8(v_b2 + off1, v1.b2);
                vst1q_u8(v_b3 + off1, v1.b3);
                vst1q_u8(v_b4 + off1, v1.b4);
            }
            for (; r < R_REGS; ++r) {
                ...
                vst1q_u8(v_b0 + off, v.b0);
                vst1q_u8(v_b1 + off, v.b1);
                vst1q_u8(v_b2 + off, v.b2);
                vst1q_u8(v_b3 + off, v.b3);
                vst1q_u8(v_b4 + off, v.b4);
            }
        }
        ...
        for (int k = 1; k < block_rows; ++k) {
            ...
            for (; r + 1 < R_REGS; r += 2) {
                ...
                v0 = slide_v5_h3_h3(v0, ho0, hi0);
                v1 = slide_v5_h3_h3(v1, ho1, hi1);
