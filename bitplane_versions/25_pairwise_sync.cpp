...
int main(int argc, char* argv[])
{
    ...
    // vb25: pairwise neighbour sync via per-thread atomic gen counters.
    // Each thread t depends only on t-1 and t+1 (torus wrap), because step_rows_bitplane
    // for rows [y0, y1) reads rows [y0-2, y1+1] of src. Those border rows are owned by
    // the two neighbour threads. Eliminating the global barrier removes ping-pong on
    // the barrier cacheline at every generation (10000 gens × T threads).
    ...
    auto worker_loop = [&](unsigned t) {
        ...
        for (int gen = 0; gen < generations; ++gen) {
            if (gen > 0) {
                // Wait until both neighbours have finished gen-1 (their counter >= gen).
                // We use yield in the spin loop on aarch64 to hint to the SMT/buffer
                // pipelines that we're idle.
    ...
    worker_loop(0);
    // Wait for all workers to reach generations before stopping the clock.
    for (unsigned t = 1; t < T; ++t) {
    ...
    // Pick final grid based on parity: after `generations` gens, the result is in
    // grid_a if generations is even, grid_b if odd. Match the (cur, next) semantics
    // used by the original loop where `std::swap(cur, next)` after each gen leaves
    // `cur` pointing at the most recent output.
