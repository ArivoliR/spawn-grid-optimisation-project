// Bitplane version 08: NEON vertical slide with EOR3 and persistent scratch.
//
// Representation:
...
// output rows.
//
// Compared to bitplane version 07, full-adder XOR chains use ARM SHA3 EOR3
// when available, and each worker reuses its scratch buffers across generations.
...
struct ThreadScratch {
    ...
    std::vector<uint64_t> rowsum_store;
    ...
    {
        adult_tmp.resize(rw);
        rowsum_store.resize(5 * 3 * (size_t)rw);
...
static void step_rows_bitplane(const BitGrid& src, BitGrid& dst, int y0, int y1, ThreadScratch& scratch)
{
    ...
    uint64_t** rs0 = scratch.rs0;
    uint64_t** rs1 = scratch.rs1;
    uint64_t** rs2 = scratch.rs2;
    ...
    auto fill_slot = [&](int src_y, int slot) {
        ...
        if (rw == 1) {
            row_sum_5_word(adult_tmp, rw, 0, rs0[slot][0], rs1[slot][0], rs2[slot][0]);
            return;
        }
