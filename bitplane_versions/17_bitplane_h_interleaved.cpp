// Bitplane version 17: vb16 + H scratch interleaved layout + apply_rule_byte micro-opts.
//
// vb16 had V scratch interleaved (v0[r],v1[r],v2[r],v3[r],v4[r] adjacent per
// register) but kept H scratch in a plane layout:
//   h0 plane for H row i: h_data + (3*i+0)*row_bytes
//   h1 plane for H row i: h_data + (3*i+1)*row_bytes
//   h2 plane for H row i: h_data + (3*i+2)*row_bytes
//
// For N=32768, row_bytes=4096. The three h0/h1/h2 vectors for a given register
// r are 4096 bytes apart — each access crosses to a different cache line, and
// the 4096-byte stride lands every access in the same cache set, causing set
// conflicts and destroying prefetcher effectiveness.
//
// vb17 applies the same interleaving principle to H scratch:
//   New H layout: h0[r], h1[r], h2[r] adjacent (48 bytes per register per row)
//   hp(i)       = h_data + i * 3 * row_bytes   (base of H row i)
//   h[i][r].h0  at hp(i) + r*48
//   h[i][r].h1  at hp(i) + r*48 + 16
//   h[i][r].h2  at hp(i) + r*48 + 32
//
// Total scratch size unchanged: (BLOCK_ROWS+4)*3*row_bytes bytes. The three
// 16-byte vectors for a register now land in 3 consecutive cache lines (or
// one 64-byte line if the alignment works out), compared to three separate
// cache lines 4096 bytes apart in vb16.
//
// In the slide loop (hot path), each iteration loads 6 vectors (ho, hi for
// one register): 6 adjacent 16-byte loads vs 6 loads with 4096-byte stride.
//
// Also applied: two micro-optimisations to apply_rule_byte that save 1-2
// instructions per call:
//
//   1. Remove nc1 = vmvnq_u8(v.b1); born_b uses vbicq_u8(v.b2, v.b1)
//      directly. Saves one MVNI instruction per call (~2.5 M calls/gen).
//
//   2. next_high: vxor3_u8(high, low, adult_r) replaces
//      vorrq_u8(veorq_u8(high, low), adult_r).
//      Proof: adult_r requires both low=1 AND high=1, so high^low=0 wherever
//      adult_r=1. Hence (high^low)|adult_r == high^low^adult_r. On sha3-
//      capable CPUs, vxor3_u8 folds to a single EOR3 instruction.
//
//   3. next_low: vbcaxq_u8(adult_r, high|born, low) replaces
//      vorrq_u8(vbicq_u8(high|born, low), adult_r).
//      Proof: adult_r requires low=1; (high|born)&~low requires ~low; the two
//      terms are disjoint, so OR == XOR == BCAX on those inputs.
//      a ^ (b & ~c) == a | (b & ~c) when a & (b & ~c) == 0.
//      Guarded with #if defined(__ARM_FEATURE_SHA3).
//
// Build (best known block size):
...
// Default block size:
//   g++-14 -std=c++23 -Ofast -mcpu=neoverse-v2+sha3 -pthread \
//          bitplane_versions/17_bitplane_h_interleaved.cpp -o /tmp/vb17_b128
