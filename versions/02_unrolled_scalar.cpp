// Version 02: single-threaded reference with neighbour counting inlined/unrolled.
// Change from 01_reference.cpp: remove count_adults() and the nested dx/dy loops.
// Still uses modulo wrapping and std::vector indexing.
...
{
    for (int y = 0; y < N; ++y) {
        const int ym2 = (y - 2 + N) % N;
        const int ym1 = (y - 1 + N) % N;
        const int yp1 = (y + 1) % N;
        const int yp2 = (y + 2) % N;

        for (int x = 0; x < N; ++x) {
            const int xm2 = (x - 2 + N) % N;
            const int xm1 = (x - 1 + N) % N;
            const int xp1 = (x + 1) % N;
            const int xp2 = (x + 2) % N;
