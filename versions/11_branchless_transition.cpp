// Version 11: v10 plus branchless transition logic.
//
// Change from 10_interior_edge_split.cpp:
//   - Replace the transition lookup table with compare/select logic.
//   - This is friendlier to compiler autovectorization and explicit SIMD.
//
// Previous v10 change from 09_hsum_lut.cpp:
//   - Build hsum edge columns with toroidal wrap.
//   - Build hsum interior columns with direct indexing, avoiding & mask in the
//     hot x=2..N-3 range.
//
// Original v09 changes from 08_sliding_counts.cpp:
//   - Build hsum[y][x] = adults in row y from x-2..x+2 for every cell.
//   - Compute A from five hsum rows, subtracting center adult.
//   - Replace the transition switch with transition logic.
//
...
static inline uint8_t next_cell_branchless(uint8_t cell, int A)
{
    const uint8_t empty_birth = (uint8_t)((cell == EMPTY) & (A >= 3) & (A <= 5));
    const uint8_t egg_grows = (uint8_t)(cell == EGG);
    const uint8_t juvenile_grows = (uint8_t)(cell == JUVENILE);
    const uint8_t adult_survives = (uint8_t)((cell == ADULT) & (A >= 4) & (A <= 9));
    return (uint8_t)(empty_birth * EGG +
                     egg_grows * JUVENILE +
                     (juvenile_grows | adult_survives) * ADULT);
}

static inline uint8_t wrapped_hsum5(const uint8_t* row, int N, int mask, int x)
{
    return (uint8_t)(adult_at(row, (x - 2 + N) & mask) +
                     adult_at(row, (x - 1 + N) & mask) +
                     adult_at(row, x) +
                     adult_at(row, (x + 1) & mask) +
                     adult_at(row, (x + 2) & mask));
}
...
static void build_hsum_rows(const uint8_t* src, uint8_t* hsum, int N, int y0, int y1)
{
    ...
    for (int y = y0; y < y1; ++y) {
        ...
        if (N <= 4) {
            for (int x = 0; x < N; ++x) {
                out[x] = wrapped_hsum5(row, N, mask, x);
            }
            continue;
        }

        out[0] = (uint8_t)(adult_at(row, N - 2) + adult_at(row, N - 1) +
                           adult_at(row, 0) + adult_at(row, 1) + adult_at(row, 2));
        out[1] = (uint8_t)(adult_at(row, N - 1) + adult_at(row, 0) +
                           adult_at(row, 1) + adult_at(row, 2) + adult_at(row, 3));

        for (int x = 2; x < N - 2; ++x) {
            out[x] = (uint8_t)(adult_at(row, x - 2) + adult_at(row, x - 1) +
                               adult_at(row, x) + adult_at(row, x + 1) +
                               adult_at(row, x + 2));
        }

        out[N - 2] = (uint8_t)(adult_at(row, N - 4) + adult_at(row, N - 3) +
                               adult_at(row, N - 2) + adult_at(row, N - 1) +
                               adult_at(row, 0));
        out[N - 1] = (uint8_t)(adult_at(row, N - 3) + adult_at(row, N - 2) +
                               adult_at(row, N - 1) + adult_at(row, 0) +
                               adult_at(row, 1));
    }
}
                                ...
                                uint8_t* dst, int N, int y0, int y1)
{
    ...
    for (int y = y0; y < y1; ++y) {
        ...
        for (int x = 0; x < N; ++x) {
            ...
            out[x] = next_cell_branchless(cell <= ADULT ? cell : EMPTY, A);
        }
    }
}
